...
class RideRepositoryImpl @Inject constructor(
    ...
    override fun observeRides(): Flow<List<Ride>> =
        rideDao.getAllRides().map { entities -> entities.map { it.toDomain() } }

    override suspend fun getRide(rideId: Long): Result<Ride> = runCatching {
        rideDao.getRideById(rideId)?.toDomain()
            ?: throw NoSuchElementException("Ride not found: id=$rideId")
    }

    override suspend fun getRideTelemetry(rideId: Long): Result<List<RideTelemetry>> = runCatching {
        telemetryDao.getTelemetryByRideId(rideId).map { it.toDomain() }
    }

    override suspend fun updateRideTitle(rideId: Long, title: String): Result<Unit> = runCatching {
        rideDao.updateTitle(rideId, title)
    }

    override suspend fun deleteRide(rideId: Long): Result<Unit> = runCatching {

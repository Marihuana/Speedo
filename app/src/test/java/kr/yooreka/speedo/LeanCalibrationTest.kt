...
private class FakeGravityRepository(
    ...
    private val _dataStream = MutableStateFlow(initial)
    override val dataStream: StateFlow<GravityData> = _dataStream
    ...
    override fun start() {
        startCount++
        emitOnStart?.let { _dataStream.value = it }
    }

    override fun stop() {
        stopCount++
        _dataStream.value = GravityData() // 실제 GravitySensor.stop() 과 동일하게 (0,0,0) 초기화
    }
}
...
class LeanCalibrationTest {
    ...
    @Test
    fun `calibrate while sensor already streaming snapshots current roll without toggling sensor`() =
        runBlocking {
    ...
    @Test
    fun `calibrate while sensor off starts samples then stops`() =
        runBlocking {

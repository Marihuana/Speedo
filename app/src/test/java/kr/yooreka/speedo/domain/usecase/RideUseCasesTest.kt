...
class RideUseCasesTest {
    ...
    @Test
    fun `GetRideHistoryUseCase exposes the repository ride stream`() = runTest {
    ...
    @Test
    fun `GetRideDetailUseCase delegates success`() = runTest {
    ...
    @Test
    fun `GetRideDetailUseCase propagates failure`() = runTest {
    ...
    @Test
    fun `GetRideTelemetryUseCase delegates to repository`() = runTest {
        val points = listOf(
            RideTelemetry(rideId = 1, timestamp = 1, speed = 5f, roll = 0f, brakeEvent = BrakeEvent.NONE, brakeForce = 0f, latitude = null, longitude = null),
        )
    ...
    @Test
    fun `UpdateRideTitleUseCase forwards arguments and returns result`() = runTest {
    ...
    @Test
    fun `UpdateRideTitleUseCase propagates repository failure`() = runTest {
        repository.updateTitleResult = Result.failure(IOException("boom"))
        assertTrue(UpdateRideTitleUseCase(repository)(1, "x").isFailure)
    }

    @Test
    fun `DeleteRideUseCase forwards id and returns result`() = runTest {
    ...
    @Test
    fun `DeleteRideUseCase propagates repository failure`() = runTest {

...
| F-13d | 음영 구간 속도 보간 (목표) | 음영 진입/진출 전후 GPS 데이터 | 보간된 속도 데이터 | **목표 설계**. F-13c 보간으로 음영 구간(터널 등)에도 path(경로 선)는 그려지나, 해당 구간 행은 속도값이 없어 속도가 표기/기록되지 않는다. 음영 **진입 직전 GPS 앵커**와 **진출 직후 GPS 앵커**의 두 픽스 사이 거리/경과시간으로 **평균속도를 역산**(`평균속도 = 앵커 간 거리 ÷ 앵커 간 경과시간`)하여, 그 사이 위치-null(속도 미수신) 구간 행들에 동일 평균속도를 채워 표기·기록한다. 기존 렌더링/데이터 흐름을 해치지 않는 범위에서 구현한다. | (신규 — To-Be) |

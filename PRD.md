...
- 앱 실행 → Splash 화면 표시 후 메인으로 전환. 근거: `MainActivity.kt:85-94`, `SplashScreen.kt`
  ...
  - **권한 게이트 ✅ 구현됨(옵션 A — 위치만 게이트)**: **위치(FINE/COARSE)** 만 핵심 권한으로 간주한다. 위치 거부 시 메인으로 진행하지 않고 "위치 권한이 있어야 앱을 사용할 수 있습니다" 안내를 표시한 뒤 앱을 종료한다. 위치 허용 시 2,000ms 후 메인 진입. **알림(POST_NOTIFICATIONS)은 게이트하지 않는다**(거부해도 진행 — 알림 없이도 핵심 동작 가능, Android 관행 부합). 근거: `SplashScreen.kt`
...
| F-07 | TPMS 표시 | BLE TPMS 광고 패킷(앞/뒤) | 앞/뒤 압력·온도·전압 + 경고색 | **⚠️ 이번 버전 비활성화 ✅ 구현됨**: 대시보드 TPMS 카드 제거, 설정의 TPMS 토글·센서 등록/초기화·압력 단위 제거, BLE 권한 런타임 요청 제거, 대시보드의 BLE 스캔(`tpmsRepository.start()`) 미가동. **백엔드/데이터 모델은 보존**(`TpmsData`/`TpmsDataSource`/`TpmsRepositoryImpl`·DataStore 키·`TPMSCard`/다이얼로그 파일 유지 — 추후 재도입 대비). | `DashBoardViewModel.kt`(TPMS 분리), `DashBoardScreen.kt`/`SettingsScreen.kt`(노출 제거) |
...
| F-13 | 주행 상세 로그 | rideId | 지도 경로 + 요약/구간 텔레메트리 | Log 화면. 상단 80% Google Map에 주행 경로를 구간별 색상 Polyline(width 12)으로 표시, 카메라는 전체 경로 bounds에 맞춤. 하단 시트는 기본 SESSION SUMMARY 표시. 지도 탭 시 가장 가까운 점 선택 → SEGMENT TELEMETRY 표시. **[사용성 보정]** ✅ 구현됨: ① 지도 어디를 탭하든 **가장 가까운 경로 점을 선택**(터치 타깃을 사실상 전체 맵으로 확장). ② 세그먼트 시트에 **앞/뒤(◀ PREV / NEXT ▶) 이동 버튼**으로 점 단위 정밀 이동. ③ **드래그-스크럽 마커**: 마커를 끌면 지도 팬 대신 마커가 이동하며 가장 가까운 경로 점으로 스냅 선택(`Marker(draggable)` + `isDragging` 스냅). | `LogViewModel.kt`(selectNext/Previous), `LogScreen.kt`(onMapClick/SegmentNavButton/드래그 스크럽) |
| F-13a | 경로 시각화 개선 | speed, roll(부호 있음) | 구간 색상 + 선 스타일 | ✅ 구현됨. **뱅킹각(Roll)**: 선의 색상으로 표기 (어두운 계열). <15°: Deep Green(`0xFF15803D`), <30°: Dark Amber(`0xFFB45309`), <45°: Dark Orange(`0xFF9A3412`), ≥45°: Deep Red(`0xFF7F1D1D`). 시인성 위해 흰색 아웃라인 언더레이.<br>**속도(Speed)**: 선의 스타일(삼각형 스탬프, `StyleSpan`+`TextureStyle`)로 표기. <100km/h: 실선, <200km/h: 희소 삼각형(`ic_path_arrow`), ≥200km/h: fast-forward(삼각형 2개 겹침, `ic_path_arrow_fast`). **화살표 방향**: 스탬프 비트맵을 상하 반전해 실제 주행 진행 방향을 가리키도록 정합(최종 방향은 실기기 Maps 렌더로 확인 권장). | `LogScreen.kt`(routeColor/speedStyle/directionStamp) |
...
| F-18 | 주행 종료 예상 감지 | 기록 중 속도 스트림, 설정 임계값 | 종료 확인 알림/팝업 | ✅ 구현됨. 기록 중 **속도 < 7km/h(정지+도보 포함, 내려서 걷는 경우 대응)** 가 설정 시간(T분) 연속 지속되면 '주행 종료 예상'. <br>1. **알림**(백그라운드 대응): "주행이 종료되었습니까?" + [종료]/[계속] 액션(알림 권한 있을 때). <br>2. **다이얼로그**: 앱 포그라운드면 확인 팝업 표시. <br>[종료]→기록 종료(F-09)+광고, [계속]→감지 타이머 초기화, 무응답→기록 유지·알림 상주. 속도 ≥7km/h 회복 시 타이머 리셋. | `TelemetryRepositoryImpl`(checkAutoStop), `RecordingService`(알림), `AutoStopDialog` |
| F-18a | 종료 감지 임계값 설정 | 사용자 선택 (ON/OFF + 3/5/10분) | 저장된 임계값 | ✅ 구현됨. 설정에서 **ON/OFF 스위치 + (ON일 때) 3/5/10분 선택**(기본 5분). 내부적으로 분 단위 정수(0=OFF)로 DataStore 저장(`autoStopThreshold`). | `UserPreferencesRepository`, `SettingsScreen`(AutoStopCard) |
...
- 권한(매니페스트 선언): ACCESS_FINE/COARSE_LOCATION, BODY_SENSORS, FOREGROUND_SERVICE(+_LOCATION), POST_NOTIFICATIONS, INTERNET, BLUETOOTH/_ADMIN/_SCAN(neverForLocation)/_CONNECT, BILLING, SYSTEM_ALERT_WINDOW. 근거: `AndroidManifest.xml:5-18`
  - 런타임 권한 요청 플로우(확정):
    - 위치(FINE/COARSE) + 알림(Android 13+): Splash 진입 시 요청. **위치 거부 시 안내 후 앱 종료**(핵심 권한 게이트). 알림 거부는 진행을 막지 않음(옵션 A, §2.1·§4.6).
...
### 4.6 권한 거부
- ✅ **구현됨(옵션 A)**: Splash에서 위치+알림 요청. **위치(FINE/COARSE) 거부 시** 안내 표시 후 앱 종료(메인 진입 차단). 알림 거부는 진행 허용. 근거: `SplashScreen.kt`
- **목표(To-Be) 확정**:
  - 위치 거부 → "위치 권한이 있어야 앱을 사용할 수 있다"는 안내 표시 후 앱 종료(메인 진입 차단). 알림 거부 → 진행 허용(게이트 안 함).
...
- **감지 타이머 리셋**: 속도가 **7km/h 이상**으로 회복되는 즉시 감지 타이머를 초기화한다(정지·도보 < 7km/h는 '종료 예상'으로 누적, 내려서 걷는 경우 대응). [계속] 선택 시에도 타이머를 초기화하며, 이후 다시 T분간 저속이면 재발행한다.
- **포그라운드/백그라운드 병행**: 앱 포그라운드면 다이얼로그를 표시하고, 백그라운드/화면 꺼짐에는 액션 알림([종료]/[계속])으로 확인한다(알림 권한 있을 때). 양쪽 모두 동일 상태(`autoStopSuggested`)에 연동되어 한쪽에서 응답하면 함께 해제된다.
...
7. ~~런타임 권한~~ — ✅ **구현됨(옵션 A)**: **위치** 거부 시 안내 후 종료(핵심 게이트), **알림은 게이트 안 함**(재검토 결과 위치만 차단). BLE는 TPMS 비활성화로 미요청 (§3.3, §4.6).
